...
- **Mass Assignment** — campos que no deberías poder enviar (API3)
- **JWT attacks** — manipular tokens de autenticación
- **API5** — operaciones de admin sin ser admin (`PUT /api/Products`)
- **API6 / Business Logic** — precio negativo, cupones vencidos en el JS
...
## Cupones vencidos — secrets en el JS

Los cupones de Juice Shop están **hardcodeados en el JavaScript del frontend**.

```
1. DevTools → Sources → main.js
   Buscar: "coupon" o "WMNSDY" o "ORANGE"

   Encontrarás algo como:
   {code: "WMNSDY2019", discount: 75, validOn: 1551999600000}
   {code: "ORANGE2024", discount: 10, validOn: ...}
```

El código de validación también está en el JS:

```javascript
// Validación del cupón — lado cliente
...
## Demo — Aplicar cupón vencido

```
Opción 1 — Interceptar en Burp y saltear validación de fecha:
  POST /rest/basket/1/coupon/WMNSDY2019
  → El backend no re-valida la fecha → aplica el 75% off

Opción 2 — Manipular el JS en DevTools:
  DevTools → Sources → main.js (pretty print)
  Buscar la función de validación de fecha
  Cambiar: if (coupon.validOn < Date.now())
  Por: if (false)
  → Todos los cupones vencidos se aceptan en el frontend

Opción 3 — Leer cupones vigentes del JS y aplicarlos directamente
  Sin pasar por la UI — request directo con Burp
```

**Enseñanza:** la validación de cupones nunca debe ser solo cliente-side.
El backend debe verificar vigencia independientemente.
